...
class SmartThermostatApp(QWidget):
    ...
    def on_message(self, client, userdata, msg):
        ...
        if "data_generation_time" in payload:
            self.data_generation_time = payload["data_generation_time"]
            self.data_generation_time_edit.setText(str(self.data_generation_time))
            self.log(f"Настройки применены. Время генерации логов: {self.data_generation_time}")
        if "desired_temperature" in payload:
            self.desired_temperature = payload["desired_temperature"]
            self.desired_temperature_edit.setText(str(self.desired_temperature))
            self.log(f"Настройки применены. Желаемая температура: {self.desired_temperature}")
        if "desired_humidity" in payload:
            self.desired_humidity = payload["desired_humidity"]
            self.desired_humidity_edit.setText(str(self.desired_humidity))
            self.log(f"Настройки применены. Желаемая влажность: {self.desired_humidity}")
        if "desired_power_conditioner_temperature" in payload:
            self.current_power_conditioner_temperature = payload["desired_power_conditioner_temperature"]
            self.desired_power_conditioner_temperature_edit.setText(str(self.current_power_conditioner_temperature))
            self.log(f"Настройки применены. Желаемая мощность кондиционера (температура): {self.current_power_conditioner_temperature}")
        if "desired_power_conditioner_humidity" in payload:
            self.current_power_conditioner_humidity = payload["desired_power_conditioner_humidity"]
